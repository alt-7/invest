...
class Admin extends Model
{
    ...
    public function historyList($route)
    {
        ...
        if (!empty($result)) {
            foreach ($result as $key => $val) {
                ...
                $account            = $this->db->row('SELECT login, email FROM accounts WHERE id = :uid', $params)[0];
    ...
    public function withdrawTariffsList()
    {
        $arr    = [];
        $result =
            $this->db->row('SELECT * FROM tariffs WHERE UNIX_TIMESTAMP() >= unixTimeFinish AND sumOut != 0 ORDER BY id DESC');
        if (!empty($result)) {
            foreach ($result as $key => $val) {
                $arr[$key]           = $val;
                $params              = [
