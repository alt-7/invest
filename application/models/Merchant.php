...
class Merchant extends Model
{
    ...
    public function validatePerfectMoney($post, $tariff)
    {
        ...
        if ($post['PAYMENT_UNITS'] != 'USD') {
            ...
            return false;
        } elseif ($amount > $tariff[$tid]['max'] || $amount < $tariff[$tid]['min']) {
            return false;
        }
    ...
    public function createTariff($data, $tarif)
    {
        ...
        $this->db->query('INSERT INTO tariffs ( uid, sumIn, sumOut, percent, unixTimeStart, unixTimeFinish) 
                        VALUES (:uid, :sumIn, :sumOut, :percent, :unixTimeStart, :unixTimeFinish)', $params);
