...
class Account extends Model
{
    ...
    public function validate($input, $post)
    {
        ...
        foreach ($input as $val) {
            if (!isset($post[$val]) || !preg_match($rules[$val]['pattern'], trim($post[$val]))) {
    ...
    public function register($post)
    {
        ...
        $this->db->query('INSERT INTO accounts ( email, login, wallet, password, ref, refBalance, token, status) 
                        VALUES (:email, :login, :wallet, :password, :ref, :refBalance, :token, :status)', $params);
    ...
    public function reset($token)
    {
        ...
        $this->db->query('UPDATE accounts SET status = 1, token = "", password = :password WHERE token = :token',
            $params);
